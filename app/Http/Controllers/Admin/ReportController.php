...
class ReportController extends Controller
{
                ...
                'orders' => function ($query) use ($from, $to, $filter) {
                    $query->when(isset($from) && isset($to) && $from != null && $to != null && $filter == 'custom', function ($query) use ($from, $to) {
                        ...
                        ->whereHas('order', function ($query) {
                            return $query->whereIn('order_status', ['completed', 'delivered', 'refund_requested', 'refund_request_canceled']);
                        });
                },
            ])
            ->withSum([
    ...
    public function hub_report(Request $request)
    {
        $key = isset($request['search']) ? explode(' ', $request['search']) : [];
        ...
        $stations = Station::with(['items', 'items.orders', 'zone'])
        ->when($request->query('module_id', null), function ($query) use ($request) {
                return $query->whereHas('store.module', function ($query) use ($request) {
                    $query->where('id', $request->query('module_id'));
                });
            })
            ->when(count($key), function ($query) use ($key) {
                return $query->where(function ($q) use ($key) {
        ...
        // Calculate order counts for each station
        foreach ($stations as $station) {
            $station->order_count = $station->items->sum(function ($item) {
                return $item->orders->count();
            });
        }

        return view('admin-views.report.low-stock-report', compact('zone', 'stations'));
    }
    ...
    public function low_stock_search(Request $request)
    {
        ...
        $stations = Station::with(['items', 'items.orders'])
            ->when($request->query('module_id', null), function ($query) use ($request) {
                return $query->whereHas('store.module', function ($query) use ($request) {
                    $query->where('id', $request->query('module_id'));
                });
            ...
            return response()->json([
                ...
                'view' => view('admin-views.report.partials._station_table', compact('stations'))->render()
            ]);
    }
