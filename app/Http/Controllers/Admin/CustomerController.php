...
class CustomerController extends Controller
{
    ...
    public function kyc_status_update(Request $request, $user_kyc, $is_verified)
    {
        try {
            // Find the existing Userkyc model by user_id
            $user_kyc_model = Userkyc::where('user_id', $user_kyc)->first();

            // Check if the Userkyc model exists
            if ($user_kyc_model) {
                // Update the is_verified field
                $user_kyc_model->is_verified = $is_verified;
                $user_kyc_model->save();

                Toastr::success(translate('messages.customer') . translate('messages.status_updated'));
            } else {
                Toastr::error(translate('messages.user_kyc_not_found'));
            }
        } catch (\Exception $e) {
            Toastr::warning(translate('messages.update_failed'));
        }
    ...
    public function view($id)
    {
        ...
        if (isset($customer)) {
            ...
            return view('admin-views.customer.customer-view', compact('customer', 'orders', 'user_kyc', 'user_kyc'));
        }
