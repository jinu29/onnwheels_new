...
class ProductLogic
{
    ...
    public static function get_zone($zone_id)
    {
         ...
         $items = Item::active() // Assuming you want only active items
        ->whereHas('store', function ($query) use ($zone_id) {
            $query->where('zone_id', $zone_id); // Assuming the zone ID is stored in the store table
        })
        ->get();
    ...
    public static function get_latest_products($zone_id, $limit, $offset, $type, $min = false, $max = false, $product_id = null)
    {
        if ($category_id != 0) {
